...
export default function Home() {
  ...
  const portals = [
    {
      ...
      desc: "Visit the official webpage of the Computer Science Department.",
      icon: (
  ...
  return (
    <main className="min-h-screen font-sans selection:bg-comsats-blue selection:text-white">
      ...
      <nav className="fixed w-full z-50 transition-all duration-300 bg-white/80 backdrop-blur-xl border-b border-slate-200/50 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <a href="#hero" className="flex items-center group">
            <img src="/logo.png" alt="COMSATS Logo" className="h-24 w-auto mix-blend-multiply group-hover:scale-105 transition-transform object-contain" />
          </a>
          ...
          <div className="hidden md:flex gap-8 items-center font-medium text-sm tracking-wide">
            ...
            <a href="/admin/login" className="px-5 py-2.5 bg-comsats-blue text-white rounded-full hover:bg-comsats-blue-dark hover:shadow-lg hover:shadow-comsats-blue/30 transition-all transform hover:-translate-y-0.5 flex items-center gap-1.5">
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-slate-200 shadow-xl py-4 px-6 flex flex-col gap-4">
            ...
            <a href="/admin/login" className="font-medium text-slate-500 text-sm flex items-center gap-2" onClick={() => setIsMenuOpen(false)}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full py-20">
          <div className="max-w-4xl">
            ...
            <h1 className="text-6xl md:text-8xl font-extrabold text-white tracking-tighter leading-[1.05] mb-8 text-balance drop-shadow-2xl">
              Computer Science <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-teal-300">Department</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-4">
              <a href="#portals" className="px-8 py-4 bg-white text-comsats-blue font-bold rounded-full hover:bg-blue-50 hover:scale-105 transition-all shadow-xl shadow-white/10 flex items-center gap-2">
                Explore Portals
      ...
      {/* Portals Section */}
      <section id="portals" className="py-32 bg-slate-50 relative">
        <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-slate-300 to-transparent"></div>
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-6xl font-bold text-slate-900 tracking-tight mb-6">Quick Access Portals</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {portals.map((portal, index) => (
              <a
                ...
                className="group bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 hover:shadow-[0_8px_30px_rgb(31,92,169,0.12)] hover:border-comsats-blue/20 transition-all duration-300 hover:-translate-y-2 flex flex-col h-full"
              >
                <div className="w-16 h-16 bg-blue-50 text-comsats-blue rounded-2xl flex items-center justify-center mb-6 group-hover:bg-comsats-blue group-hover:text-white transition-colors duration-300">
                  {portal.icon}
                </div>
                <h3 className="text-2xl font-bold text-slate-900 mb-3 tracking-tight">{portal.title}</h3>
                <p className="text-slate-600 leading-relaxed flex-grow">{portal.desc}</p>
                <div className="mt-6 flex items-center text-comsats-blue font-semibold text-sm tracking-wide group-hover:gap-2 transition-all">
                  Open Portal
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {news.length > 0 && (
            <div className="mb-20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {news.map(item => (
                  <Link href={`/news/${item.id}`} key={item.id} className="group bg-slate-50 rounded-3xl overflow-hidden border border-slate-100 hover:shadow-[0_8px_30px_rgb(31,92,169,0.12)] hover:border-comsats-blue/20 transition-all duration-300 hover:-translate-y-1 cursor-pointer">
          ...
          {events.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {events.map(item => (
                  <Link href={`/events/${item.id}`} key={item.id} className="group flex gap-5 bg-slate-50 rounded-2xl p-6 border border-slate-100 hover:shadow-[0_8px_30px_rgb(31,92,169,0.12)] hover:border-comsats-blue/20 transition-all duration-300 cursor-pointer">
      ...
      <section id="newsletter" className="py-32 relative overflow-hidden bg-slate-900">
        ...
        >
          <source src="https://latdncjdcwtmtehhmazi.supabase.co/storage/v1/object/public/COMSATS%20CS%20PORTAL%20ASSETS/3129595-hd_1280_720_30fps%20(1).mp4" type="video/mp4" />
        </video>
